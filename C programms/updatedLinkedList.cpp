...
#define SIZE 3 // size of list

struct Data { 
	char letter; // value used in for data part of list
};
...
void viewAllNodes(); //viewing nodes to see if any duplicates 
bool duplicate(); // searches through list to check for duplicate values
...
void main() {
	...
	//View all nodes and sees if there any duplicate values
	viewAllNodes();

	if (duplicate())
		printf("There are duplicates in the list");
	else
...
bool duplicate() {
	...
	struct LinearNode *current, *compare;
	...
	if (isEmpty())
		...
		while (current != NULL) {
			...
			while (current != NULL) {
				if (current->element->letter == compare->element->letter)
					return true;
